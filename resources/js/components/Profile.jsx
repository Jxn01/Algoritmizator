 ...
 * It includes sections for viewing user details, statistics, and successful attempts.
 * Users can update their name, username, email, and password through a modal form.
...
const Profile = memo(({ title, activeTab }) => {
    ...
    // Fetch user data and successful attempts on component mount
    useEffect(() => {
    ...
    // Go back to form selection in modal
    ...
    const validateForm = () => {
        ...
        if (selectedForm === 'email') {
            if (!formData.email.trim()) errors.email = "Az e-mail cím megadása kötelező.";
            if (!emailRegex.test(formData.email)) errors.email = "Csak inf.elte.hu e-maileket fogadunk el.";
            if (!formData.password.trim()) errors.password = "A jelszó megadása kötelező.";
        }
    ...
    // Render form based on selected form type
    const renderForm = () => {
        if (!selectedForm) {
    ...
    // Render selection buttons for editing profile details
    const renderSelectionButtons = () => (
        <>
            <button type="button" onClick={() => openModal('name')}
                    className="block w-full text-center bg-purple-700 py-2 rounded-md hover:bg-purple-800 transition duration-300 text-white">
    ...
    // Render text field for name and username
            ...
            <div className="mb-4">
                <label htmlFor="newPassword" className="block text-white text-sm font-bold mb-2">
                    ...
                    <ReactTooltip anchorSelect={'#newPassTip'} place="right" effect="solid">
                        Minimum 8 karakter hosszú, legalább egy kis- és nagybetűt, egy számot és egy speciális karaktert tartalmazzon.
                    </ReactTooltip>
                </label>
                <input
