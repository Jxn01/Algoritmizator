 ...
 * Socials component
 *
 * This component renders a social interaction interface with tabs for viewing friends, friend requests, and searching for users.
        ...
        <div>
            ...
            <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-r from-purple-600 via-purple-700 to-purple-800">
                <div className="w-full max-w-2xl mt-5 text-center">
                    {/* Tabs for switching between Friends, Requests, and Search */}
                    <div className="flex space-x-2 mb-4">
                        <button onClick={() => setActiveSocialTab('friends')}
                                className={`px-4 py-2 text-white rounded-lg hover:bg-gray-800 transition duration-300 ${activeSocialTab === 'friends' ? 'bg-gray-800' : 'bg-purple-800'}`}>
                            Barátok
                        </button>
                        <button onClick={() => setActiveSocialTab('requests')}
                                className={`px-4 py-2 text-white rounded-lg hover:bg-gray-800 transition duration-300 ${activeSocialTab === 'requests' ? 'bg-gray-800' : 'bg-purple-800'}`}>
                            Kérelmek
                        </button>
                        <button onClick={() => setActiveSocialTab('search')}
                                className={`px-4 py-2 text-white rounded-lg hover:bg-gray-800 transition duration-300 ${activeSocialTab === 'search' ? 'bg-gray-800' : 'bg-purple-800'}`}>
                            Keresés
                        </button>
                    </div>
                    {/* Conditionally render the appropriate component based on the active tab */}
 ...
 * FriendsComponent
 *
 * This component fetches and displays a list of friends. It allows the user to view friend details and remove friends.
 * @returns {JSX.Element} The friends component
 */
const FriendsComponent = memo(() => {
                        ...
                        <div key={friend.id}
                            ...
                            <div>
                                <a href={`/algoritmizator/app/socials/profile/${friend.id}`}
                                   className="px-6 py-2 mr-3 bg-purple-700 text-white rounded-lg hover:bg-purple-800 transition duration-300">
                                    Profil
                                </a>
 ...
 *
 * This component fetches and displays a list of friend requests. It allows the user to accept or reject friend requests.
 * @returns {JSX.Element} The friend requests component
 */
const FriendRequestsComponent = memo(() => {
                        ...
                        <div key={request.id}
                            ...
                            <div>
                                ...
                                <button onClick={() => handleAccept(request.id)}
                                        className="px-4 py-2 mr-3 bg-green-600 rounded-lg hover:bg-green-700 transition duration-300">
                                    Elfogadás
                                </button>
 ...
 * SearchComponent
 *
 * This component allows users to search for other users and send friend requests.
                                ...
                                <div>
                                    ...
                                    {!user.is_friend &&
                                        <button disabled={user.friend_request_sent || user.friend_request_received}
                                                ...
                                                className={`px-4 py-2 ${user.friend_request_sent || user.friend_request_received ? 'bg-gray-600' : 'bg-green-600 hover:bg-green-700 transition duration-300'} rounded-lg`}>
                                            {user.friend_request_sent ? 'Barátkérelem elküldve' : user.friend_request_received ? 'Barátkérelem fogadva' : 'Barátkérelem küldése'}
                                        </button>
                                    }
                                </div>
                            </div>
